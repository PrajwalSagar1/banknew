...
const Transfer: React.FC = () => {
    ...
    const handleTransfer = async (e: React.FormEvent) => {
        ...
        if (recipientUid === user?.uid) {
            return setStatus({ type: 'error', message: 'You cannot transfer funds to your own UID.' });
        }
        if (Number(amount) > (user?.balance || 0)) {
            return setStatus({ type: 'error', message: 'Insufficient balance for this transfer.' });
        }
        ...
        try {
            ...
            if (response.data.success) {
                setStatus({
                    type: 'success',
                    message: `Transfer of ₹${amount} to ${recipientUid} was successful!`
                });
            ...
            setStatus({
                type: 'error',
                message: error.response?.data?.message || 'Transfer failed. Please check the recipient UID and your balance.'
            });
        } finally {
            setLoading(false);
        }
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="px-4 py-12 mx-auto max-w-3xl sm:px-6 lg:px-8">
                <div className="mb-10 animate-in fade-in slide-in-from-left-4 duration-500">
                    ...
                    <h1 className="text-4xl font-black text-gray-900 tracking-tight">Move Wealth</h1>
                    <p className="text-gray-500 font-medium">Instant internal transfers using bank-grade security.</p>
                </div>
                    ...
                    <form onSubmit={handleTransfer} className="space-y-8">
                        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
                            <div className="space-y-2">
                                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest">Recipient Identifier (UID)</label>
                                <input
                                    type="text"
                                    required
                                    placeholder="e.g. UKB1002"
                                    value={recipientUid}
                                    onChange={(e) => setRecipientUid(e.target.value)}
                                    className="w-full px-5 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 focus:bg-white transition-all outline-none font-mono font-bold text-gray-900"
                                />
                                <p className="text-[10px] text-gray-400">UID is case-sensitive.</p>
                            </div>
                            <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                            <label className="block text-xs font-black text-gray-400 uppercase tracking-widest">Transaction Memo</label>
                            <textarea
                                rows={3}
